...
const copy: Record<Mode, { eyebrow: string; title: string; subtitle: string; action: string }> = {
  ...
  register: { eyebrow: "受邀注册", title: "创建协作账号", subtitle: "注册需要有效邀请码，账号权限由管理员统一配置。", action: "注册并进入" },
...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  return <div className="mx-auto w-full max-w-md rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
    ...
    <form className="mt-7 space-y-4" onSubmit={submit}>
      ...
      {mode === "register" && <><Field name="inviteCode" label="邀请码" autoComplete="off" required /><Field name="displayName" label="姓名" autoComplete="name" required /><Field name="username" label="用户名（字母、数字、点、横线）" autoComplete="username" required /><Field name="email" label="邮箱" type="email" autoComplete="email" required /><Field name="password" label="密码（至少10位，含字母、数字和符号）" type="password" autoComplete="new-password" required /></>}

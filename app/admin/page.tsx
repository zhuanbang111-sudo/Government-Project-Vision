...
type Invite = { id: string; code_hint: string; role: string; department_name?: string; max_uses: number; used_count: number; expires_at: string; status: string; remark?: string; public_display: number; public_code?: string; public_label?: string };
...
export default function AdminPage() {
  ...
  async function createInvite(event: FormEvent<HTMLFormElement>) {
    ...
    try { const response = await fetch("/api/admin/invitations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(values) }); const result = await response.json() as { code?: string; error?: string; publicDisplay?: boolean }; if (!response.ok) throw new Error(result.error || "创建失败"); setMessage(result.publicDisplay ? `内部共享邀请码已发布到注册页：${result.code}` : `邀请码（仅显示一次）：${result.code}`); event.currentTarget.reset(); setPublicRegistration(false); await load(); }
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <section className="rounded-xl border bg-white p-5"><h2 className="font-bold">创建邀请码</h2><p className="mt-1 text-xs text-slate-500">专属邀请码仅显示一次；内部共享邀请码会直接显示在注册页。</p><form onSubmit={createInvite} className="mt-4 grid gap-3 sm:grid-cols-2">
        <label className="flex items-start gap-3 rounded-lg border border-teal-100 bg-teal-50 p-3 text-xs sm:col-span-2"><input name="publicDisplay" type="checkbox" checked={publicRegistration} onChange={(event) => setPublicRegistration(event.target.checked)} className="mt-0.5 size-4 accent-teal-700" /><span><strong className="block text-teal-900">发布为内部共享邀请码</strong><span className="mt-1 block leading-5 text-slate-500">固定为普通用户并显示在注册页；发布新共享码时，旧共享码会自动失效。</span></span></label>
        {publicRegistration ? <div className="rounded-lg border px-3 py-2.5 text-xs text-slate-600"><span className="block font-semibold">预设角色</span><strong className="mt-1 block text-sm text-slate-900">普通用户</strong><input type="hidden" name="role" value="user" /></div> : <Select name="role" label="预设角色" options={[["user", "普通用户"], ["reviewer", "审核人"], ["admin", "管理员"]]} />}
        <Select name="departmentId" label="归属部门" options={[["", "暂不分配"], ...departments.filter((item) => item.status === "active").map((item) => [item.id, item.name])]} />
        <Field key={`uses-${publicRegistration}`} name="maxUses" label="可使用次数" type="number" defaultValue={publicRegistration ? "50" : "1"} min="1" max="100" />
        <Field key={`days-${publicRegistration}`} name="expiresInDays" label="有效天数" type="number" defaultValue={publicRegistration ? "30" : "7"} min="1" max="90" />
        {publicRegistration && <Field name="publicLabel" label="注册页提示名称" defaultValue="内部试用邀请码" maxLength={60} />}
        <Field name="remark" label="备注" /><button disabled={pending} className="self-end rounded-lg bg-teal-700 px-4 py-2.5 text-xs font-bold text-white">{publicRegistration ? "发布共享邀请码" : "生成邀请码"}</button>
      </form></section>
    ...
    <section className="rounded-xl border bg-white p-5"><h2 className="font-bold">邀请码记录</h2><div className="mt-3 grid gap-2">{invites.map((invite) => <div key={invite.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-slate-50 p-3 text-xs"><span className="flex items-center gap-2 font-mono font-bold">{invite.public_display === 1 ? invite.public_code : invite.code_hint}{invite.public_display === 1 && <span className="rounded-full bg-teal-100 px-2 py-1 font-sans text-[10px] text-teal-800">注册页公开</span>}</span><span>{invite.public_label || invite.role} · {invite.department_name ?? "未分配"}</span><span>{invite.used_count}/{invite.max_uses} · {invite.status}</span><span>截至 {new Date(invite.expires_at).toLocaleDateString("zh-CN")}</span></div>)}</div></section>
  </div>;
}

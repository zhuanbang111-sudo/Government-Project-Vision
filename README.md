...
- 项目负责人可以邀请工作区成员，并分别授予查看、编辑或审核权限。
- 编辑人员将指定文稿版本提交审核；系统同时核对正文引用标记与该版本的来源快照，生成引用核验清单。
- 审核人员可以按章节或段落添加建议、重要问题和阻断问题，并将批注标记为已解决或重新打开。
- 存在未解决的阻断问题或缺失来源时不能批准定稿；退回后可保存修订版本并再次送审。
- 审核通过后系统自动生成最终版本，保留版本差异、审核决定、引用检查和操作日志，最终 DOCX 可归档到 R2。

## 项目归档中心

- “写作项目”按进行中、已完成和已归档分开展示，并统计各状态项目数量。
- 归档项目保留任务信息、参考资料、全部版本、审核记录、引用核验结果和 R2 中的 DOCX，详情页以只读方式开放。
- 项目负责人可以恢复归档项目；恢复状态会根据最终稿、待审核任务、已有版本和参考资料自动判断。
- 可以从归档项目复制任务信息、提纲和参考资料配置，创建新的续写项目，原档案不会被修改。
- 归档列表可以直接下载最近一次 DOCX 导出；永久删除暂不开放，以保持审计链完整。
